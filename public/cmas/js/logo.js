	...
	require.prototype = {
		_init: function(b) {
			...
			$('body').on({
					...
					focus: function(a) {
						b.onFocus ? b.onFocus($(this)) : $(this).parent().find("label.focus").not(".valid").removeClass("hide").siblings(
							".valid").addClass("hide") && $(this).parent().find(".blank").addClass("hide") && $(this).parent().find(
							".close").addClass("hide")
					},
					keyup: function(a) {
						if (b.phone && $(this).attr("id") === "phone") d._change($(this))
...
normal.onclick = function() {
	...
	normal.style.borderBottom = "1px solid #F3414D";
	nopw.style.color = "#B3B3B3";
	normal.style.color = "#2B3341";
	rcFlag = true;
	lcFlag = false;
...
nopw.onclick = function() {
	...
	sm.style.display = "none";
	regTop.style.display = "block";
	nopw.style.borderBottom = "1px solid #F3414D";
	normal.style.borderBottom = "none";
	nopw.style.color = "#2B3341";
	normal.style.color = "#B3B3B3";
	rcFlag = false;
	lcFlag = true;
}
	...
	if (reg.test(this.value) || reg1.test(this.value) || reg2.test(this.value)) {
		...
		$.post('./js/login.php', {
			...
			if (data == '0') {
				...
				$('#rc-inner-num > span').text('鐢ㄦ埛鍚嶄笉瀛樺湪锛岃閲嶆柊杈撳叆锛�');
			} else {
				$('.success').show();
				nFlag = true;
			}
...
inputPassword.onblur = function() {
	if (this.value == "") {
		if (nFlag) {
			...
			rcInnerNum.style.display = "none";
			rcInnerVirityText.innerText = "璇疯緭鍏ュ瘑鐮�";
		}
		return;
	}
	...
	if (!reg.test(this.value)) {
		rcInnerVirity.style.display = "block";
		rcInnerVirityText.innerText = "瀵嗙爜涓�8-16浣嶇殑瀛楁瘝鎴栨暟瀛楁垨鐗规畩瀛楃鐨勭粨鍚�";
		rcInnerNum.style.display = "none";
	} else {
		passFlag = true;
		return;
...
loginBtn.onclick = function(e) {
	...
	if (inputPhone.value == "") {
		rcInnerNum.style.display = "block";
		rcinnerText.innerText = "璇疯緭鍏ユ樀绉�/閭/鎵嬫満鍙风爜";
		rcInnerVirity.style.display = "none";
		return;
	}
...
noPhoneNum.onblur = function() {
	...
	if (this.value == "") {
		...
		innerNumText.innerText = "璇疯緭鍏ユ墜鏈哄彿鐮�";
		innerVirity.style.display = "none";
		return;
	}
	if (!reg.test(this.value)) {
		innerNum.style.display = "block";
		innerNumText.innerText = "璇疯緭鍏ユ纭殑鎵嬫満鍙风爜";
		innerVirity.style.display = "none";
		return;
	} else {
		var value = $(this).val();
		$.post('./js/login.php', {
			...
			if (data == '0') {
				$('#inner-num').show().text();
				$('#inner-virity').hide();
				$('#inner-num > span').text('鎵嬫満鍙风爜涓嶅瓨鍦�,璇烽噸鏂拌緭鍏ワ紒');
			} else {
				$('.success').show();
				npnFlag = true;
			}
...
otp.onblur = function() {
	...
	if (this.value == "") {
		if (npnFlag) {
			...
			innerVirityText.innerText = "璇疯緭鍏ラ獙璇佺爜";
			innerNum.style.display = "none";
		}
		return;
	}
	if (!reg.test(this.value)) {
		if (npnFlag) {
			innerVirity.style.display = "block";
			innerVirityText.innerText = "楠岃瘉鐮佷负6浣嶆暟瀛�";
			innerNum.style.display = "none";
		}
	} else {
		otpFlag = true;
