$(function() {
	...
	var zym = 'http://www.ynresearch.net';
	...
	$.ajax({
		...
		success: function(data) {
			...
			html += '<div class="theory_p1"><span class="author">' + data.data.content.author +
				'</span> <a class="Title" href="/sj.html?">实践</a><span class="time">' + data.data.content.crea_at +
				'</span></div><h1>' +
				data.data.content.title + '</h1><div class="Article">' + data.data.content.free_content + '</div>';
			$('.sectionc ').html(html);
			// 相关文章
