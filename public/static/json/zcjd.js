	...
	$.ajax({
		...
		success: function(data) {
			...
			$.each(data.data.activity, function(k, v) {
				var activity = '';
				activity +=
					'<li><a id="activity' + v.id + '">' + v.title + '</a><b>作者：<span>' + v.author + '</span></b></li>'
				activity += '</div>';
				...
				$("#activity" + v.id + "").click(function() {
					...
					var time = v.crea_at;
					var message = v.message;
					location.href = './hdwz.html?id=' + ID + '?' + 'time' + time;
				})
			})
			// 右边
			// 猜你喜欢
			...
			$.each(data.data.like, function(k, v) {
				...
				like +=
					'<li><a href="' + likeurl + '' + v.id + '" id="like' + v.id + '"  target="_blank">' + v.title +
			...
			$.each(data.data.technigue, function(k, v) {
				console.log(v)
				var technigue = '';
				technigue += '<li><a id="technigue' + v.id + '">' + v.title + '</a><b>作者：<span>' + v.author +
					'</span></b></li>'
				// Frontier += '<a href="./theory.html" class="ckgd">查看更多</a> '
				$('.sectionR_twoUL2  .ul .ckgd ').before(technigue);
