...
<div class="x-nav">
    ...
    @foreach($column as $v)
        <span  data-column_id="reload" style="margin-left: 30px;" class="fa-clock-o" id="{{ $v -> id }}">{{ $v -> column }}</span>
    @endforeach
...
<div class="x-body">
    ...
    <xblock >
        <button class="layui-btn" onclick="x_admin_show('添加文章','/home/article/Add')"><i class="layui-icon"></i>添加</button>
    ...
    layui.use(["jquery", "upload", "form", "table", "layer", "element", "laydate"], function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: "title", width: 455, title: "标题", align: "left"},
                {field: "author", width: 200, title: "作者", align: "left"},
                {field: "column", width: 200, title: "分类", align: "left"},
                {field: "view", width: 80, title: "浏览量", align: "center" ,sort: true, totalRow: true},
                {field: "crea_at", width: 160, title: "创建时间", align: "left" , sort: true, totalRow: true},
        ...
        $(document).on('click','span.fa-clock-o',function(){
        ...
        table.on('tool(userTables)', function (obj) {
            ...
            switch (obj.event) {
                case 'show':
                    ...
                    //return false;
                    //layer.msg('查看');
                    x_admin_show('查看文章','/home/article/Show?id='+ data.id + '&column=' + column);
                    break;
                case 'del':
                    //layer.msg('删除');
                    layer.confirm('真的删除么', function (index) {
                        $.ajax({
                            url:'/home/article/Del',
                            type:'get',
                            dataType:'json',
                            data:{id:data.id},
                            headers: {
                    ...
                    break;
                case 'edit':
                    x_admin_show('编辑文章','/home/article/Edit?id=' + data.id);
                    break;
            }
            ;
        })
