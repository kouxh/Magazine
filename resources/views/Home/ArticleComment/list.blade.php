...
{{--        <button class="layui-btn" onclick="x_admin_show('添加课堂','/home/classroom/Add')"><i class="layui-icon"></i>添加</button>--}}
...
<script type="text/html" id="toolbarDemo">
    <div class="layui-btn-container">
        <a title="通过"  lay-event="DoEdit" href="javascript:;">
    ...
    layui.use(["jquery", "upload", "form", "table", "layer", "element", "laydate"], function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: "title", width: 290, title: "文章标题", align: "left"},
                {field: "account", width: 150, title: "用户昵称", align: "left"},
                {field: "com_comment", width: 355, title: "评论内容", align: "left"},
                {field: "com_status", width: 105, title: "评论状态", align: "left"},
                {field: "com_crea_at", width: 160, title: "评论时间", align: "left" , sort: true, totalRow: true},
                {fixed: 'right', title: '操作', width: 110, align:'center',toolbar: '#toolbarDemo'}
            ]],
            url: "/home/articleComment/Page",
            headers:{'COLUMN' : 1},
            // headers: {
            //     'Column': column
            // },
            ...
            switch (obj.event) {
                ...
                case 'DoEdit':
                    layer.confirm('确定通过么？', function (index) {
                        $.ajax({
                            url:'/home/articleComment/DoEdit',
                            type:'get',
                            dataType:'json',
                            data:{id:data.com_id},
                            success:function(res){
                                if(res.err_code === 10000){
                                    layer.close(index);
                                    layer.alert('通过成功' , function(){
                                        location.reload();
                                    });

                                }else{
                                    layer.close(index);
                                    layer.alert('通过失败' , function(){
                                        location.reload();
                                    });
                                }
                            }
