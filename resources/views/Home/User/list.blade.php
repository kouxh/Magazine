...
<script type="text/html" id="toolbarDemo">
    <div class="layui-btn-container">
        <a title="重置密码" lay-event="resetpwd" href="javascript:;"   >
            <i class="layui-icon layui-icon-password" style="margin-left: -4px"></i>
        </a>
        <a title="修改积分"  lay-event="editintegral" href="javascript:;">
            <i class="layui-icon layui-icon-diamond" style="margin-left: 10px"></i>
        </a>
        <a title="修改余额"  lay-event="editbalance" href="javascript:;">
        <i class="layui-icon layui-icon-rmb" style="margin-left: 10px"></i>
        </a>
...
<script>
    ...
    layui.use(["jquery", "upload", "form", "table", "layer", "element", "laydate"], function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                {field: "account", width: 250, title: "用户名", align: "left"},
                {field: "tell", width: 250, title: "手机号", align: "left"},
                {field: "integral", width: 80, title: "积分", align: "left" ,},
                {field: "balance", width: 160, title: "余额", align: "left" ,},
                {field: "is_vip", width: 110, title: "会员状态", align: "left" , templet:'#status'},
                {field: "crea_time", width: 160, title: "注册时间", align: "left" , },
        ...
        $('#searchEmailCompany').on('click', function () {
            ...
            active[column_id] ? active[column_id].call(this) : '';
        });     //搜索
    ...
    layui.use('table', function() {
        ...
        table.on('tool(userTables)', function (obj) {
            ...
            switch (obj.event) {
                case 'resetpwd':
                    layer.confirm('真的需要重置密码么？', function (index) {
                        $.ajax({
                ...
                case 'editintegral':
                    ...
                    layer.open({
                        ...
                        ,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">'+
                            ...
                            '                <input type="text" id="integral" name="all_price"' +
                            '                       autocomplete="off" class="layui-input" value="'+data.integral+'" placeholder="例：0.00">\n' +
                            '            </div>\n' +
                        ...
                        ,yes:function (index,layero) {
                            $.ajax({
                                url:"/home/user/editIntegral",
                                type:'post',
                                data:{id:data.id, integral:$("#integral").val()},
                                dataType:'json',
                                success:function(res){
                ...
                case 'editbalance':
                    layer.open({
                        ...
                        ,content: '<div style="padding: 50px; line-height: 22px; background-color: #393D49; color: #fff; font-weight: 300;">\n' +
                            ...
                            '                    </label>\n' +
                            '                    <div class="layui-input-inline" style="width: 220px;">\n' +
                            '                        <input type="text" id="balance" name="logistics_code" required="" lay-verify=""\n' +
                            '                               autocomplete="off" class="layui-input" value="'+data.balance+'" placeholder="例：123456879">\n' +
                            '                    </div>\n' +
                            '                </div>\n' +
                            '            </div>'
                        ,yes:function (index,layero) {
                            $.ajax({
                                url:"/home/user/editBalance",
                                type:'post',
                                data:{id:data.id, balance:$("#balance").val()},
                                dataType:'json',
                                success:function(res){
                                    if (res.bol == true && res.err_code == 10000) {
                                        layer.alert(res.msg , function(){
